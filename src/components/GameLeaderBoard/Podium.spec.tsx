...
describe("Podium Component", () => {
  ...
  test("applies correct class names based on team position", () => {
    render(<Podium teams={teams} onClick={mockOnClick} />);
    expect(screen.getByText("podium.1st").closest(".podium-card")).toHaveClass(
      "gold",
    );
    expect(screen.getByText("podium.2nd").closest(".podium-card")).toHaveClass(
      "silver",
    );
    expect(screen.getByText("podium.3rd").closest(".podium-card")).toHaveClass(
      "bronze",
    );
  });
});
